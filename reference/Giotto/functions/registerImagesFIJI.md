# `registerImagesFIJI` {#registerImagesFIJI}
...
Wrapper function for Register Virtual Stack Slices plugin in
FIJI
...
- `transforms_save_dir`: (jython implementation only) Folder to save
transforms to
- `ref_img_name`: (jython implementation only) File name of reference
...
- `DryRun`: Whether to return the command to be run rather than actually
executing it.
...
This function was adapted from runFijiMacro function in
