...
Create Giotto object directly from a 10X visium folder. Also
...
- `h5_image_png_path`: path to tissue .png file (optional). Image
...
- `do_manual_adj`: deprecated
- `xmax_adj`: deprecated
- `xmin_adj`: deprecated
- `ymax_adj`: deprecated
- `ymin_adj`: deprecated
- `instructions`: list of instructions or output result from
`[createGiottoInstructions](GiottoClass)`
- `expression_matrix_class`: class of expression matrix to use
(e.g. "dgCMatrix", "DelayedArray")
- `h5_file`: optional path to create an on-disk h5 file
...
- expr_data: raw will take expression data from
  raw_feature_bc_matrix and filter from filtered_feature_bc_matrix
- gene_column_index: which gene identifiers (names) to use if there
