...
Creates a known spatial pattern for selected genes one-by-one
...
- `spatial_probs`: probabilities to test for a high expressing gene
...
- `spat_methods_params`: list of parameters list for each element in the
vector of spatial methods to test
- `spat_methods_names`: name for each element in the vector of spatial
elements to test
