# `findMarkers_one_vs_all` {#findMarkers_one_vs_all}
...
- `min_expr_gini_score`: gini: filter on minimum gini coefficient for
expression
...
- `adjust_columns`: mast: column in pDataDT to adjust for
(e.g. detection rate)
- `verbose`: be verbose
- `...`: additional parameters for the findMarkers function in scran or
...
`[findGiniMarkers_one_vs_all]` and
