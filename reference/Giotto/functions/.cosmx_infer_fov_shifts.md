# `.cosmx_infer_fov_shifts` {#.cosmx_infer_fov_shifts}
...
- `tx_dt`: transcript data.table input to use
(Only one of tx_dt or meta_dt should be used)
- `meta_dt`: cell metadata data.table input to use
(Only one of tx_dt or meta_dt should be used)
- `flip_loc_y`: whether a y flip needs to be performed on the local y
values before comparing with global y values. See details
- `navg`: max n values to check per FOV to find average shift
