...
1. calculate gini-coefficient for av. expression values over all clusters
1. calculate gini-coefficient for detection fractions over all clusters
1. convert gini-scores to rank scores
1. for each feat create combined score = detection rank x expression rank x
expr gini-coefficient x detection gini-coefficient
1. for each feat sort on expression and detection rank and combined score
...
it is recommend to provide names for both groups to *group_1_name* and
*group_2_name*
