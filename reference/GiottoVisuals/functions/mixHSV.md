...
```r
mixHSV(c1, c2, base_color = c("white", "black"), output = c("hex", "hsv"))
```

## Arguments

- `c1, c2`: Colors 1 and 2. Accepts vector of hex color codes or an hsv
matrix
- `base_color`: either "white" (default) or "black". Which color to treat
as the absence of signal in `c1` and `c2`
- `output`: either "hex" or "hsv". "hex" produces a vector of hex codes
for color mixtures. "hsv" returns the hsv matrix.

## Value

a vector of hex codes or an hsv matrix
