...
**SUPERSEDED** *`[createGiottoLargeImage()](=createGiottoLargeImage)` is preferred for most
...
left.
- `do_manual_adj`: (default = FALSE) flag to use manual adj values instead
of automatic alignment when given a gobject or spatlocs
- `xmax_adj, xmin_adj, ymax_adj, ymin_adj`: adjustment of the maximum or
...
- `xmin_set, xmax_set, ymin_set, ymax_set`: values to override image minmax
...
[**flip_x_axis**] flip x-axis (`[image_flop](magick)`)
[**flip_y_axis**] flip y-axis (`[image_flip](magick)`)
