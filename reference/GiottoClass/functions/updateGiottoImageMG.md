...
Updates the boundaries of a giotto `image` object attached
to a `giotto` object if both `gobject` and `image_name`
...
- `return_gobject`: return a `giotto` object if `TRUE`, a
giotto `image` object if `FALSE`
- `verbose`: be verbose

## Value

a `giotto` object or an updated giotto `image` object
