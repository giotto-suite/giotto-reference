...
Write full resolution image to file. Filetype extension should
be included in `filename` argument. Be careful if write time and disk
...
- `max_intensity`: (optional) image max intensity value from
which `dataType` can be automatically determined
