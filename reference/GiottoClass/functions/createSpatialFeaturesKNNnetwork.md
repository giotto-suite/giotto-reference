# `createSpatialFeaturesKNNnetwork` {#createSpatialFeaturesKNNnetwork}
...
- `add_feat_ids`: add feature id
names (default = FALSE, increases object size)
- `verbose`: be verbose
- `return_gobject`: return giotto object (default: TRUE)
- `toplevel_params`: toplevel value to pass when updating giotto params
- `...`: additional parameters to pass to `[kNN](dbscan)`

## Value

If `return_gobject = TRUE` a giotto object containing the
network will be returned. If `return_gobject = FALSE` the network
