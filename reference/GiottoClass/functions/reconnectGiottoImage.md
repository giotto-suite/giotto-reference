...
- `largeImage_path`: named list of paths to images to reconnect to
giottoLargeImages
- `verbose`: be verbose
...
Inputs can either be given as both image
name (`image_name`/`largeImage_name`) and
filepath (`image_path`/`largeImage_path`) args or as only a named
list through a filepath argument alone. If `auto_reconnect = TRUE` then
...
image handling package objects (*magick* or *raster objects*) or
